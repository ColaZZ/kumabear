...
def get_user_followed_posts(id):
    user = User.query.get_or_404(id)
    page = request.args.get('page', 1, type=int)
    pagination = user.posts.order_by(Post.timestamp.desc()).painate(
        page, per_page=current_app.config['KUMABEAR'],
        error_out=False)
    posts = pagination.items
    prev = None
    if pagination.has_prev:
    ...
    next = None
    if pagination.has_next:
        next = url_for('api_get_user_followed_posts', page=page+1, _external=True)
    return jsonify({
        'posts': [post.to_json() for post in posts],
        'prev': prev,
        'next': next,
